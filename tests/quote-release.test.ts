...
test("Phase 1 keeps every Phase 2 mutation and future route closed", () => {
  execFileSync(
    ...
    for(const action of ['profile.enableProvider','provider.save','template.save','quote.enable','quote.submit','quote.select','quote.chat','selection.accept','trade.confirm','review.create'])
