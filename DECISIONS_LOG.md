...
## 2026-09-20 — 구조화된 1차 제안과 공통 답변
사용자 승인: 확정·예상·확인 후 제안, 필요한 경우만 채팅, 공통 질문은 고객이 공유에 동의한 답변을 제안 참여 업체에게 제공. 공개 원문에 민감 답변을 추가하지 않는다. 최종 확정 제안만 선택하고 기존 참여자는 모집기간 종료 뒤에도 최종 견적을 수정할 수 있다. 기존 100% 표시는 이전 기능 범위에 한정하며 이번 요구사항은 별도 추가 범위다. docs/PROPOSAL_WORKFLOW.md 참조. 운영 공개/SQL/push 승인 없음.
