...
## 최신 — 구조화된 제안·공통 추가정보
- 확정/예상/확인 후 폼·가격 비교·최종 전환·공통 질문/답변·참여자 권한 구현. SQL0018 준비. 단위·DB 46개, PC/모바일 E2E 6개, 타입·lint·프로덕션 빌드 통과. 운영 미적용.
- 상세 정책: docs/PROPOSAL_WORKFLOW.md. 이전 완료율은 이전 요구 범위 기준.

## 이전 — Phase 2 개발·로컬 검증 완료
...
- 로컬 3107 경로 확인, 견적 전환 PC/모바일 E2E 2개 및 타입/린트 통과. 원격 push·운영 SQL·배포는 수행하지 않음.

## 최신 — Phase 2A 견적 요청 전환
- 기존 해줘요 글을 명시적 동의 후 원래 URL/내용/사진/댓글/채팅 연결을 유지하며 견적 요청으로 전환한다.
- 전환 시점부터 72시간/최대120시간, 중복 및 일반 수정 우회 방지. SQL0015와 Supabase 요청 RPC·집계 매핑 준비, 원격 적용은 미실행.
- 이번 작업 범위 2A 완료. Phase 2 원격 공개 준비는 6개 완료 묶음 중 1개, **약 17%**. 전체 프로젝트 완성률이 아니다.
- 운영 Phase 1 유지, 원격 Phase 2 제한과 DB 공개 플래그 false 유지. 상세 근거와 다음 작업: [PHASE2_PROGRESS](docs/PHASE2_PROGRESS.md).
...
## 2026-09-20 알림 미리보기 및 채팅 종료
알림에 작성자/본문 최대100자(화면2줄) 표시. 새 알림은 source_id로 정확한 메시지/댓글 연결, 과거 알림은 접근 가능한 선행 내용 참고. 알림 투명 backdrop hover 공통 배경색 오류 수정. 대화 종료는 양쪽 전송 영구 차단/안내 표시, 종료 후 내 목록 숨기기 분리. SQL0009는0008 정의도 포함하여 단독 적용 가능. DB trigger가 직접 insert 및 종료와의 경합도 검사. 자동 삭제는 미구현, 보관정책 결정 후 진행. 로컬/DB30개 및2계정 PC/모바일E2E 통과. 원격 SQL 적용 대기.


## 2026-09-20 MY 댓글 활동
마이페이지 내가 쓴 글과 나의 대화 사이에 내가 쓴 댓글 수/목록 전환 추가. 본인 댓글 최신순, 원글 제목/댓글 내용/작성일 표시 및 원글 댓글영역 이동. 삭제/숨김 글과 숨김 댓글 제외. 기존 snapshot 사용, SQL 추가 없음.


## 2026-09-20 Cloudflare 날짜 배포 오류 수정
사용자 제공 UTC2026-09-19 로그에서 OpenNext build 및 assets upload 성공 확인. 배포는 미래 compatibility_date 2026-09-20 오류10021로 실패. wrangler 날짜를2026-09-19로 수정. 재배포 성공은 미확인. 환경변수 보존 설정 유지.
...
## 2026-09-20 댓글 신고/차단 배치
댓글 하단 오른쪽에 신고/차단 묶음 배치. 본인 댓글은 삭제만, 로그인한 타인 댓글에는 확인 후 작성자 차단 제공. 관리자 버튼은 닉네임 옆 유지. 기존 block.create 사용, SQL 없음. 타입/린트 및 댓글 관리자 회귀 E2E 통과.


## 2026-09-20 글 신고/차단 접힘 메뉴
채팅하기는 왼쪽 유지, 신고/작성자차단은 우측 깃발 아이콘의 접힘 메뉴로 이동. 메뉴 밖 포커스/ESC/항목선택 시 닫힘. PC1440/모바일390 숨김·펼침·ESC 및 가로넘침 검증, 타입/린트 통과.


## 2026-09-20 댓글 메뉴 압축
댓글 닉네임 줄 우측 점3개 아이콘으로 신고/차단 및 본인 삭제 메뉴 통합. 하단 액션 줄 제거로 여백 축소. 관리자 삭제는 닉네임 옆 유지. 메뉴 선택/포커스이탈/ESC 닫힘.


## 2026-09-20 프로필 미리보기 정렬
프로필 수정 미리보기의 inline-flex 덮어쓰기를 inline-grid 중앙 정렬로 수정. 업로드 사진은 block 및 object-position:center 지정, 기본 아이콘/사진 동일한 원형 정렬.


## 2026-09-20 업로드 버튼 정리
기본 파일선택 UI를 투명 input 오버레이로 숨겨 업로드 버튼 하나로 표시. 기존 파일선택/accept/업로드 처리 유지, 키보드 포커스 테두리 및 disabled 표시 추가. 프로필/게시글 공통 적용.


## 2026-09-20 다중 사진 및 운영팀 샘플
게시글 사진 다중 선택 지원, 기존 사진 포함 최대 5장. 초과 선택은 남은 수량만 추가. 브라우저에서 2장 추가 후 6장 선택 시 총 5장 제한 확인.
0010_sample_posts.sql: 청소/온라인 웹 제작 샘플 각 1개, 운영팀 표시 및 댓글/채팅 차단. 관리자 소유권 유지, 고정 ID로 재실행 중복 방지. SQL은 준비 상태이며 원격 적용 필요. SQL 테스트 포함 30개 통과.


## 2026-09-20 가이드 자동 초안·SEO·사업자 커뮤니티
- 하루 1개 AI 초안 서버 경로, 날짜 잠금/최대3회 재시도/실패 기록. 관리자 편집·검수·발행·보류. 기본 gpt-4o-mini, 실시간 검색 없음.
- 가이드 DB/RLS와 커뮤니티 사업자 분류 SQL0011 준비(원격 미적용). Secrets/예약 Worker 배포는 사용자 설정 필요. 자동 공개 없음.
- 홈 검색 제목 고정, 홈/공개 가이드 검색 허용 설정·canonical·구조화 데이터·공통 브랜드 OG 이미지·사이트맵. 로컬은 noindex 유지. 가이드 서버 렌더링 검증.
- 생활·개인/사업자·업체 필터, 공통 분야+안전·전기·소방·승강기시설 등 분야. 업체 소개는 별도 탭, 기본 피드/인기글 제외. 조직/RFQ 권한은 기존대로 분리.
- 검증: 타입/lint, 34개 테스트, 브라우저2개(편집 검수·noJS본문·사업자소개·모바일), Next 프로덕션 빌드 통과. 예약 Worker dry-run 번들 검증 통과. 실제 AI/원격 SQL/스케줄러 실행은 설정 후 확인 필요.
- 사용자 설정 절차: docs/GUIDE_SEO_BUSINESS_SETUP.md.

## 2026-09-20 공개 헤더 준비 중 메뉴
Phase1에서도 견적받기·업체찾기·맡다 비즈 메뉴 노출. 미출시 루트는 준비 중 안내로 연결, 하위 경로와 기능 플래그는 기존 차단 유지. 모바일 가로 스크롤 헤더 메뉴. 타입/lint 및 실제 Phase1 미리보기에서 루트3개 200/모바일 넘침 없음/RFQ404 확인.


## 2026-09-20 가이드 런타임 Secret 읽기 보완
가이드 OpenAI/API 모델·Supabase 서비스 키·예약 토큰을 요청 시 Cloudflare 바인딩에서 우선 조회하고 일반 Node 로컬은 process.env 사용. 런타임 키 인식 회귀 테스트 추가. 35개 테스트와 타입/lint 통과. 실제 도메인 생성은 배포 후 재확인 필요.


## 2026-09-20 시스템 사업자 샘플 2개
0012_business_samples.sql: 전기안전관리 대행/제품촬영 예시 각1개. 개인 계정 없이 author_id=null인 시스템 예시만 허용, 실제 글 작성자 필수 및 기존 RLS 유지. 샘플 작성자 차단 UI 제외. 재실행 중복없음/개인 없는 실제 글 거부/샘플채팅차단 포함 35개 테스트 통과. 원격 SQL 실행 필요.
